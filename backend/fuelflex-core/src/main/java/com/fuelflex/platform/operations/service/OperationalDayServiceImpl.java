...
public class OperationalDayServiceImpl implements OperationalDayService {
 ...
 @Override public DayResponse open(OpenDayRequest request){
  ...
  record("OPERATIONAL_DAY",day.getId(),OperationalAction.OPERATIONAL_DAY_OPENED,null,"OPEN",actor,"businessDate="+day.getBusinessDate()); notifier.recordOperationalDayActivity(actor,day,OperationalDayActivityType.OPERATIONAL_DAY_OPENED); return day(day);
 }
 ...
 @Override public DayResponse close(UUID id,CloseDayRequest request){User actor=manager(); OperationalDay day=lockDay(id,actor); if(day.getStatus()==OperationalStatus.CLOSED) throw new ConflictException("The operational day is already closed."); dayClosing.prepare(day,actor,request); day.setStatus(OperationalStatus.CLOSED); record("OPERATIONAL_DAY",id,OperationalAction.OPERATIONAL_DAY_CLOSED,"OPEN","CLOSED",actor,"referenceCurrency="+day.getOrganization().getDefaultCurrency()); return day(days.save(day));}
 @Override public AssignmentResponse assign(UUID dayId,OpenAssignmentRequest request){
  ...
  record("SHIFT_ASSIGNMENT",shift.getId(),OperationalAction.SHIFT_ASSIGNMENT_OPENED,null,"OPEN",actor,"meter="+meter.getId()+",openingIndex="+shift.getOpeningIndex()+",pump="+pump.getId()); notifier.recordOperationalDayActivity(actor,day,OperationalDayActivityType.SHIFT_ASSIGNMENT_OPENED); return assignment(shift);
 }
 ...
 User actor=manager(); UUID org=organizationId(actor); PumpShiftAssignment shift=shifts.lockByIdAndOrganizationId(id,org).orElseThrow(()->new ResourceNotFoundException("Shift assignment was not found.")); stationAccess.checkStationAccess(actor,shift.getOperationalDay().getStation().getId());
  ...
  record("SHIFT_ASSIGNMENT",id,OperationalAction.SHIFT_ASSIGNMENT_CLOSED,"OPEN","CLOSED",actor,"closingIndex="+closing+",meteredVolume="+shift.getMeteredVolume()); notifier.recordOperationalDayActivity(actor,shift.getOperationalDay(),OperationalDayActivityType.SHIFT_ASSIGNMENT_CLOSED); return assignment(shift);
 }
