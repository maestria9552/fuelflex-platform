...
public interface SaleStockMovementRepository extends JpaRepository<SaleStockMovement,UUID>{
 ...
 @Query("select coalesce(sum(case when m.movementType='OUTBOUND' then m.quantity else -m.quantity end),0) from SaleStockMovement m where m.tank.id=:tankId and m.sale.shiftAssignment.status='OPEN'") BigDecimal sumOutboundByTankId(@Param("tankId")UUID tankId);
}
